...
function MemberCard({
  ...
  return (
    <div className="member-card group relative flex flex-row border border-white/10 hover:border-blue-500/30 bg-[#0a0a0a] hover:bg-white/3 transition-all duration-300 overflow-hidden cursor-default h-[200px]">
      <div className="relative w-2/5 h-full overflow-hidden grayscale group-hover:grayscale-0 transition-all duration-500 shrink-0">
        ...
        />
        <div className="absolute inset-y-0 right-0 w-8 bg-linear-to-r from-transparent to-[#0a0a0a] group-hover:to-[#0d0d0d]" />
      </div>
...
const topMembers = teamMembers.filter(
  (m) => m.role === 'Chairperson' || m.role.includes('Technical Head')
);
